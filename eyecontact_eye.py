...
from stimulus_eye import eyeOpenClose_child, eyeOpenClose_parent #importing stimulus program

import os


#### Get subject info with GUI ########################################################
participant = {'participant':'', 'parent or child': ['parent', 'child']} #participant number and if parent or child
...
if participant['parent or child'] == 'child':
    expInfo = {'participant':'hyperscan_' + participant['participant'] + 'c', 'condition': 1, 'TR': 1.000, 'volumes': 400, 'sync':'4', 'ON': 20, 'OFF': 20, 'EyeTracking?': True}
else:
    expInfo = {'participant':'hyperscan_' + participant['participant'] + 'p', 'condition': 1, 'TR': 1.000, 'volumes': 400, 'sync':'4', 'ON': 20, 'OFF': 20, 'EyeTracking?': True}
...
win = visual.Window((scnWidth, scnHeight), fullscr=True, monitor=mon, color= 'black', units='pix', allowStencil=True)
...
def runTrial(window, mr_settings, trial_index, on, off, reps):
    ...
    if trial_index > 3:
        vid = load_video(participant['participant'], trial_index, window, participant['parent or child'])
    ...
    if participant['parent or child'] == 'child':
        aborted = eyeOpenClose_child(participant['participant'], window, on, off, trial_index, reps, vid) #indicates if stimulus program aborts
    else:
        aborted = eyeOpenClose_parent(participant['participant'], window, on, off, trial_index, reps, vid) #indicates if stimulus program aborts
...
while trial <= 6:
    ...
    if participant['parent or child'] == 'child':
        dataFileName = participant['participant'] +'c' + str(trial) + '.EDF'
    else:
